...
this year's World Robot Summit (WRS) in October. 
...
\begin{itemize}
\item Topological navigation - The topological navigation system from the STRANDS project was integrated into the HSR as the main navigation system used on the HSR. The system allows for the placement of predefined nodes in our map and for the creation of edges between any of the nodes placed on the map. The system also allows for the specification of an action on each edge and node, allowing the user to define any number of actions to take upon reaching or node, or whilst navigating to a node; this allows us to use the move_base configurations specified by Toyota, or to define our own navigation with optimized parameters. 
\item Manipulation and grasping
\item Verbal commands
