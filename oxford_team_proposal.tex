...
Previously, we used a hierarchical navigation system from the STRANDS project. The navigation system was structured around a topological map, in which discrete locations were connected by directed edges~\cite{jpulido2015NowOrLater}. Edges corresponded to navigation actions the robot could perform to transition between locations. These may be standard \texttt{move\_base} actions, social navigation, closed loop controllers (such as wall following or door passing), or teach-and-repeat paths. This system has since been replaced, however, we will explore incorporating elements of topological navigation into the current navigation system to permit location-based action-selection, such as checking if a door is open, checking if a human is present for greeting, and following a human from a given start location.
...
\textbf{Task-Level Planning} - The task-level planning sub-system acts as the executive controller of the robot system. Autonomous behaviours are defined through the use of hierarchical SMACH state machines, which combine abilities provided by other sub-systems, accessible via ROS Action servers. This composition of sub-system abilities permits the HSR to perform useful tasks in a domestic environment (e.g., fetching a bottle of water for a human, storing an object on a shelf, asking a human what it can do to help). The task-level planning sub-system interacts directly with the \emph{`tmc\_move\_base'} ROS package for navigation (see \nameref{sec:capability-navigation} \ref{sec:capability-navigation}).
